...
describe("terminal resize status", () => {
  it("selects one pane across PTY, recovery, and rendered boundaries", () => {
    expect(terminalResizeStatus({
      ...
      requested: { cols: 54, rows: 24 }, rendered: { cols: 54, rows: 24 },
      renderedOutputSequence: 1001, operation: "ready",
      diagnostics: {
        pty: { sessions: [
          { session: 8, paneId: "pane-b", cols: 90, rows: 24, eventSequence: 2 },
          { session: 7, paneId: "pane-a", cols: 54, rows: 24, eventSequence: 3, written: 1003 },
        ] },
        recovery: { sessions: [
          { pane: "pane-b", cols: 90, rows: 24, eventSequence: 2 },
          { pane: "pane-a", cols: 54, rows: 24, eventSequence: 3, outputSequence: 1002, gaps: 1 },
        ] },
      },
    })).toEqual({
      hostPixels: { width: 432, height: 384 }, requested: { cols: 54, rows: 24 },
      pty: { cols: 54, rows: 24, eventSequence: 3, outputSequence: 1003 },
      recovery: { cols: 54, rows: 24, eventSequence: 3, outputSequence: 1002, gaps: 1 },
      rendered: { cols: 54, rows: 24, outputSequence: 1001 }, operation: "ready",
    });
  });
  ...
  it("reports unavailable boundaries as null", () => {
    expect(terminalResizeStatus({
      ...
      rendered: null, renderedOutputSequence: null, operation: "closed", diagnostics: { pty: {}, recovery: {} },
    })).toEqual({
